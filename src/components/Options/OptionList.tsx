    ...
    if (direction === Directions.Right) {
      const notFitItem = optionsRef.current.find((item) => {
        ...
        return (
          ...
          contRef.current.getBoundingClientRect().right
        );
      });
      if (!notFitItem) return;
      const dx =
        notFitItem.getBoundingClientRect().right -
        contRef.current.getBoundingClientRect().right;
      ...
      const dx =
        contRef.current.getBoundingClientRect().left -
        notFitItem.getBoundingClientRect().left;
      setOffset((offset) => offset + dx);
    }
  };
  ...
  return (
    <div className="max-w-[90%] md:max-w-full flex gap-[20px] w-full h-full  justify-center md:flex-col">
      ...
      >
        <div
          className={clsx(
            "w-full flex gap-1  duration-200 h-full scrollbar-hide",
            {
