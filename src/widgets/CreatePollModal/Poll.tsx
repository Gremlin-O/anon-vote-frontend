...
const validatePollData = (poll: IPoll): boolean => {
	const errors:string[] = []
	if (poll.name.length===0 ) {
		errors.push("Опрос должен иметь название")
	}
	if (poll.questions.length<1 ) {
		errors.push("Опрос должен иметь хотя бы один вопрос")

	}
	if (poll.category) {
		errors.push("Опрос должен содержать категорию")

	}
	if (poll.questions.some((qst)=> qst.answers.length<2 && qst.answers.some((ans)=> !ans.text))) {
		errors.push("Каждый вопрос должен иметь хотя бы 2 непустых ответа")
	}
	return true
}
...
const Poll = () => {
  ...
  const handleAddQuestion = () => {
    ...
    setTimeout(() => {
      ...
      questionsContRef.current.scrollTop =
        questionsContRef.current.scrollHeight;
    }, 10);
  };
              ...
              return (
                <PollQestion
                  ...
                  changeQuestion={(value) =>
                    handleChangeQuestion(value, questionInd)
                  }
                  changeAnswer={(value, answerInd) =>
                    handleChangeAnswer(value, questionInd, answerInd)
                  }
                  removeQuestion={() => handleRemoveQuestion(questionInd)}
                  removeAnswer={(answerInd) =>
                    handleRemoveAnswer(questionInd, answerInd)
                  }
