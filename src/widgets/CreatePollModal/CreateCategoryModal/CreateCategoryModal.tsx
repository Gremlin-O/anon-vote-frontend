...
const CreateCategoryModal: FC<ICreateCategoryProps> = ({ show, onClose, onCategoryCreate }) => {
	...
	const handlePlusClick = useCallback((nodeId: string, event: React.MouseEvent) => {
	...
	const Node = ({ node, style, dragHandle }: any) => {
		...
		return (
			<div style={style} ref={dragHandle} className='node' onClick={() => node.toggle()}>
				<div
					className={clsx(
				...
				>
					<div className='flex items-center min-h-10'>
						...
						<span className='text-sm md:text-base text-primary'>{node.data.name}</span>
					</div>
				...
				<div className='flex flex-col justify-between items-start lg:items-center gap-4 mt-4 md:mt-5'>
					<div
						className={clsx(
							...
							{
								'bg-green-50 border-green-200 text-green-800': selectedCategoryId,
								'bg-yellow-50 border-primary text-primary': !selectedCategoryId,
							}
						)}
					>
						{selectedCategoryId ? (
							<p className='font-medium'>✅ Выбрана категория для встраивания</p>
						) : (
					...
					<div className='flex md:flex-col gap-2 md:gap-3 w-full lg:w-auto'>
						<Button onClick={handleCancel} text='Отмена' className='w-[200px] sm:w-auto text-center' />
						<Button
							onClick={handleConfirm}
							className={clsx('w-[200px] sm:w-auto text-center transition-opacity', {
